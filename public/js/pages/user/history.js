...
const deleteHistory = (id) => {
  const willDelete = confirm('Apakah anda yakin ingin menghapus video dari history?');
  if (willDelete) {
    fetch(`http://127.0.0.1:8000/api/history/delete?actualId=${id}`, {
      method: "POST",
      body: JSON.stringify({
        actualId: id,
...
const getHistory = async () => {
  ...
  if (videos.length == 0) {  // jika tidak ada history
    displayalertContainer("Anda belum mempunyai history video");
  }
  else {
    const bodyData = [];
    ...
    for (let i = 0; i < videos.length; i++) {
      ...
      trash_bin.setAttribute("onclick", `deleteHistory('${v_id}')`);
